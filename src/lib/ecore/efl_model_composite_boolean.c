                                                             ...
                                                             const char *property)
{
   ...
   if (strcmp(property, "child.index") == 0)
     return eina_value_int_new(pd->index);
...
_efl_model_composite_boolean_then(void *data, const Eina_Value v, const Eina_Future *dead_future EINA_UNUSED)
{
   ...
   unsigned int i, len;
   Eina_Value o = EINA_VALUE_EMPTY;
   Eina_Value r = eina_value_reference_copy(&v);

   if (eina_value_type_get(&r) != EINA_VALUE_TYPE_ARRAY)
     goto on_error;

   EINA_VALUE_ARRAY_FOREACH(&r, len, i, &o)
     {
        Efl_Model_Composite_Boolean_Children_Data *cpd;
        Eo *composite;
        Eo *target = NULL;

        target = eina_value_object_get(&o);
        if (!target) continue;
        ...
        cpd = efl_data_scope_get(composite, EFL_MODEL_COMPOSITE_BOOLEAN_CHILDREN_CLASS);
        cpd->parent = req->pd;
        cpd->index = req->start + i; // FIXME: should be a general children property

        eina_value_array_pset(&r, i, &composite);
     }
                                         ...
                                         const char *name, Eina_Bool default_value)
{
   ...
   value = malloc(sizeof (Efl_Model_Composite_Boolean_Value));
   if (!value) return ;
