...
{
public:
    LottieKeyFrame():mPathKeyFrame(false){}
public:
    T                   mStartValue;
    T                   mEndValue;
    int                 mStartFrame;
...
{
public:
    LottieAnimatable():mValue(),mKeyFrames(nullptr){}
    LottieAnimatable(const T &value): mValue(value), mKeyFrames(nullptr){}
    constexpr bool isStatic() const {return ((mKeyFrames==nullptr) ? true : false);}
public:
    T                     mValue;
    LottieKeyFrames<T>   *mKeyFrames;
...
public:
    void accept(LottieObjectVisitor *visitor) override
    {visitor->visit(this); visitor->visitChildren(this);}
    LottieLayer():LottieGroupObject(LottieObject::Type::Layer),mId(-1), mParentId(-1){}
