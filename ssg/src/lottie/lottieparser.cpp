...
public:
    ...
    template<typename T>
    void parseKeyFrame(LottieKeyFrames<T> &obj);
    template<typename T>
    void parseProperty(LottieAnimatable<T> &obj);

    void parseShapeKeyFrame(LottieKeyFrames<LottieShape> &obj);
 ...
 */
template<typename T>
void LottieParser::parseKeyFrame(LottieKeyFrames<T> &obj)
{
    EnterObject();
        ...
        if (0 == strcmp(key, "k")) {
            if (PeekType() == kArrayType) {
                ...
                while (NextArrayValue()) {
                    RAPIDJSON_ASSERT(PeekType() == kObjectType);
                    if (!obj.mKeyFrames)
                        obj.mKeyFrames = new LottieKeyFrames<LottieShape>();
                    parseKeyFrame(*obj.mKeyFrames);
                }
            } else {
                getValue(obj.mValue);
            }
            ...
            if (PeekType() == kNumberType) {
                ...
                while (NextArrayValue()) {
                    ...
                    if (PeekType() == kObjectType) {
                        if (!obj.mKeyFrames)
                            obj.mKeyFrames = new LottieKeyFrames<T>();
                        parseKeyFrame(*obj.mKeyFrames);
                    } else {
